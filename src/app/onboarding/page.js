...
const items = [
  // ---------------- Website Design (Direct Build) ----------------
  {
    ...
    min: 299000,
    max: 299000,
    desc: "We build a very simple website to get you started.",
    recurring: false,
  },
  {
    id: "diy_pro",
    ...
    min: 599000,
    max: 599000,
    desc: "Multi-page business website built from your supplied content and structure.",
    recurring: false,
  },
  {
    id: "diy_ecommerce",
    ...
    min: 699000,
    max: 699000,
    desc: "Store setup and payments configured. Products managed by you.",
    recurring: false,
  },
  {
    id: "diy_custom",
    ...
    min: 949000,
    max: 949000,
    desc: "We build a custom website for your specific needs.",
    recurring: false,
  },
  ...
  // ---------------- 3D Modelling Products ----------------
  {
    ...
    label: "3D Product Visual Pack",
    priceValue: 450000,
    min: 450000,
    max: 450000,
    desc: "3 still renders, white/studio BG. Commercial use.",
    recurring: false,
  },
  {
    id: "3d_social_ad",
    category: "3D Modelling",
    label: "Social Media 3D Ad",
    priceValue: 850000,
    min: 850000,
    max: 850000,
    desc: "5-10 sec animation, music, motion text.",
    recurring: false,
  },
  {
    id: "3d_brand_system",
    category: "3D Modelling",
    label: "Brand 3D System",
    priceValue: 1500000,
    min: 1500000,
    max: 1500000,
    desc: "Product model, animation kit, reusable assets.",
    recurring: false,
  },

  // ---------------- AI Products ----------------
  {
    id: "ai_support_bot_setup",
    category: "Automated Services",
    label: "Customer Support AI Bot (Setup)",
    priceValue: 249000,
    min: 249000,
    max: 249000,
    desc: "FAQ training (50 Q&A), WhatsApp integration.",
    recurring: false,
  },
  {
    id: "ai_support_bot_monthly",
    category: "Automated Services",
    label: "Support Bot Hosting",
    priceValue: 49000,
    min: 49000,
    max: 49000,
    desc: "We provide support and maintenance for your AI bot.",
    recurring: true,
  },
  {
    id: "lead_automation",
    category: "Automated Services",
    label: "Lead Automation System",
    priceValue: 599000,
    min: 599000,
    max: 599000,
    desc: "Auto-capture, auto-reply, CRM sync.",
    recurring: false,
  },
  {
    id: "ops_automation",
    ...
    min: 899000,
    max: 899000,
    desc: "Invoicing, data entry, custom workflows.",
    recurring: false,
  },
];
...
export default function QuoteBuilder() {
  ...
  const handleSubmit = (method) => {
    ...
    if (method === 'dashboard') {
        // Logic to send to dashboard (server action or API)
        console.log("Submitting to dashboard:", { contact, services: picked, totals: { oneTime, monthly } });
        alert("Request sent to Dashboard! (Simulation)");
        return;
    }
    ...
    <div className="min-h-screen bg-[var(--bg-page)] text-[var(--text-main)] font-sans selection:bg-blue-100 selection:text-blue-900">
        ...
        <main className="pt-32 pb-40 max-w-7xl mx-auto px-6 md:px-12 flex flex-col lg:flex-row gap-12 relative">
            ...
            <div className="w-full lg:w-2/3 lg:pl-12 space-y-12">
                ...
                <section>
                    <div className="mb-10">
                        <div className="bg-blue-50 border border-blue-200 rounded-none p-4 mb-8">
                            <p className="text-sm text-blue-800 font-medium">
                                <span className="font-bold">Note:</span> Direct Build packages are for clients who already know what they want.
                            </p>
                        </div>
                        ...
                        {CATEGORIES.map(cat => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`btn-category relative px-4 py-6 md:py-8 text-sm md:text-base font-bold transition-all duration-200 font-heading ${
                                        activeCategory === cat
                                        ? "active"
                                        : ""
                                    }`}
                                >
                                    {cat}
                                    {count > 0 && (
                                        <span className="absolute top-3 right-3 bg-black text-white text-[10px] font-bold px-2 py-0.5 rounded-sm shadow-sm">
                                            {count}
                                        </span>
                                    )}
                                </button>
                    ...
                    {errors.select && (
                        <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-sm mb-8">
                            {errors.select}
                        </div>
                    )}
                        ...
                        >
                            ...
                            {currentItems.map(item => (
                                <button
                                    ...
                                    className={`group relative flex flex-col md:flex-row md:items-start justify-between p-6 md:py-8 rounded-sm border transition-all duration-200 text-left ${
                                        selected[item.id]
                                ...
                                >
                                    <div className="flex-1 pr-4 mb-4 md:mb-0 w-full">
                                        <div className="flex items-center justify-between gap-3 mb-1 w-full">
                                            ...
                                            {item.recurring ? (
                                                <span className="px-2 py-0.5 rounded-sm text-[10px] md:text-xs font-bold bg-purple-100 text-purple-700 uppercase tracking-wide whitespace-nowrap">Monthly</span>
                                            ) : (
                                                <span className="px-2 py-0.5 rounded-sm text-[10px] md:text-xs font-bold bg-[#EDEBEC] text-[#636161] uppercase tracking-wide whitespace-nowrap">One-time</span>
                                            )}
                                        </div>
                                        {item.desc && (
                            ...
                            {/* 3D Modelling Items */}
                            {/* (No custom inputs needed for 3D Modelling currently) */}
                ...
                <div className="pt-12 border-t border-gray-200">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 justify-end mt-8 w-full md:w-auto">
                        {/* Primary: Direct Entry */}
                        <button 
                            onClick={() => handleSubmit('dashboard')}
                            className="bg-black text-white px-8 py-4 rounded-sm font-bold text-lg transition-colors w-full md:w-auto md:min-w-[260px] text-center shadow-lg"
                        >
                        ...
                        <div className="flex items-center w-full md:w-auto gap-3">
                            <div className="h-px bg-gray-300 flex-1 md:w-12"></div>
                        ...
                        <button 
                            ...
                            className="bg-[#25d366] text-black border-2 border-black px-8 py-3 rounded-sm font-bold text-base transition-colors w-full md:w-auto md:min-w-[260px] flex items-center justify-center gap-3 shadow-sm"
                        >
                            ...
                            <span className="leading-tight text-left">Send directly on<br/>whatsapp</span>
                        </button>
                    </div>
                </div>
