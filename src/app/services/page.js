...
const SERVICES = [
  ...
  },
  {
    ...
    alignment: "left",
    image: "/images/3d/model-face-only.png",
    details: {
      tagline: "Enter a new dimension.",
      need:
        "Flat 2D images no longer capture attention. Brands need immersive experiences to stand out.",
      benefit:
        "Photorealistic 3D assets that showcase your products from every angle and create unforgettable brand moments.",
      result:
        "Higher engagement, improved customer understanding, and a cutting-edge visual identity."
    }
  }
];
...
function ServiceItem({ service }) {
  ...
  return (
    <div
      ref={ref}
      className={` bg-[var(--bg-page)] service-item flex flex-col-reverse md:flex-row items-center md:items-start gap-12 md:gap-24 ${service.alignment === 'left' ? 'md:flex-row-reverse' : ''
        }`}
    >
      ...
      >
        ...
        {/* <div className={`absolute top-6 right-6 z-20 w-16 h-16 rounded-full bg-white/90 backdrop-blur-md shadow-lg flex items-center justify-center border border-white/50`}>
          <span className={`text-xl font-bold ${service.id === "01" ? "text-blue-600" : service.id === "02" ? "text-orange-500" : (service.id === "03" ? "text-emerald-600" : "text-purple-600")}`}>{service.id}</span>
        </div> */}
      </motion.div>
    </div>
  );
  ...
  return (
    <div id="services-page" className="bg-[var(--bg-page)] min-h-screen text-[var(--text-primary)] selection:bg-blue-100 selection:text-blue-900 overflow-x-hidden relative">
      ...
      <main className="pt-24 md:pt-32 pb-20 w-full global-padding relative z-10">
        ...
        <div id="services-hero" className="relative md:min-h-[calc(100vh-256px)] flex flex-col justify-center items-center mt-16 md:mt-8 text-center pb-20 md:pb-40 mb-12">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            <h1 className="text-5xl! md:text-8xl! mb-6 text-[var(--text-primary)] max-w-5xl relative z-10">
            ...
            <div className="mt-10 flex justify-center relative z-10">
              <Link href="/contact" className="group inline-flex items-center gap-2 bg-[var(--text-primary)] text-[var(--bg-page)] px-8 py-4 rounded-small text-lg font-medium transition-all duration-300 hover:opacity-90">
